...
:END:
   ...
   - Inclusive Mark (Alt-m)
     ...
     "Inclusive" character marking in Brief includes the character under the cursor, whereas in Emacs the marked region
     stops on the character before the cursor. This behaviour is actually Brief's "Non-inclusive Mark" (Alt-a).
     "Inclusive Mark" is simulated in Brf-mode by moving point one more position to include the initial character.
